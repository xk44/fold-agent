...
def test_redact_case_uses_current_route() -> None:
    ...
    payload = client.redact_case(
        "case-123", redaction_level="deidentify", confirm=True, reason="privacy"
    )
...
def test_redact_subject_uses_current_route() -> None:
    ...
    payload = client.redact_subject(
        "case-123", "sub-456", redaction_level="anonymous", confirm=True, reason="owner request"
    )
...
def test_redact_case_endpoint_exists() -> None:
    ...
    created_case = client.post(
        "/cases", json={"species": "demo", "diagnosis_summary": "redact route case"}
    )
...
def test_redact_subject_endpoint_exists() -> None:
    client = TestClient(app)
    created_case = client.post(
        "/cases", json={"species": "demo", "diagnosis_summary": "subject redact route case"}
    )
    case_id = created_case.json()["id"]
...
def test_delete_case_endpoint_exists() -> None:
    client = TestClient(app)
    created_case = client.post(
        "/cases", json={"species": "demo", "diagnosis_summary": "delete route case"}
    )
    case_id = created_case.json()["id"]

    response = client.delete(
        f"/cases/{case_id}", params={"confirm": True, "hard_delete": False, "reason": "cleanup"}
    )
    assert response.status_code == 200
    assert response.json()["case_id"] == case_id
