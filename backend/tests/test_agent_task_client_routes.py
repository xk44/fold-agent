...
def test_update_agent_task_endpoint_exists() -> None:
    ...
    created_case = client.post("/cases", json={"species": "demo", "diagnosis_summary": "agent update case"})
    case_id = created_case.json()["id"]

    created_task = client.post(
        "/agent/tasks",
        json={"case_id": case_id, "framework": "hermes", "skill_name": "monitor_case_progress", "status": "pending"},
    )
