"""NeoVax-Agent Report Exports (Phase 9 + Phase 17)
...
def scan_for_unsafe_content(text: str) -> list[dict]:
    ...
    for compiled, meta in _COMPILED_PATTERNS:
        for m in compiled.finditer(text):
            ...
            overlapping = any(
                not (end <= s or start >= e) for s, e in seen_spans
            )
            if not overlapping:
...
def build_report_html(case_id: str, db: "Session") -> str:
    ...
    created_str = (
        case.created_at.strftime("%Y-%m-%d %H:%M UTC")
        if case.created_at
        else "unknown"
    )
...
<head>
  ...
  <title>NeoVax-Agent Case Report — {_h(case_id)}</title>
  <style>
...
<body>
  <h1>NeoVax-Agent Case Report</h1>
  ...
  <div class="watermark">
    Generated by NeoVax-Agent &bull; Version {_h(version_str)} &bull;
...
def build_report_markdown(case_id: str, db: "Session") -> str:
    ...
    lines += [
        "---",
        f"_Generated by NeoVax-Agent — Version {version_meta['version']} — "
...
NeoVax-Agent outputs are research coordination artifacts only and do not constitute veterinary advice.
...
Sex: [SEX]
Weight: [WEIGHT]
NeoVax-Agent case reference: {case_id}

---

DESCRIPTION OF RESEARCH COORDINATION ACTIVITY

The supervising veterinarian has used NeoVax-Agent, a computational research coordination tool,
...
THIS IS NOT A TREATMENT PLAN. NeoVax-Agent does not prescribe, formulate, or administer any
...
[ ] The supervising veterinarian has reviewed and interprets all NeoVax-Agent outputs.
...
2. NeoVax-Agent provides research coordination support only — it is not a validated clinical tool.
...
- I have reviewed all NeoVax-Agent research coordination outputs for this case.
- I have discussed the experimental nature of this activity with the owner/guardian.
- No dosing, injection, or treatment protocol has been derived directly from NeoVax-Agent outputs.
...
- NeoVax-Agent is a research coordination tool only. It cannot produce approved consent forms.
...
NeoVax-Agent, a computational research coordination tool, has identified {candidate_count}
...
Note: NeoVax-Agent provides computational research coordination only. It does not specify,
...
"""
    ...
    return {
        ...
        "attestation_fields": [
            ...
            {"field": "pi_credentials", "label": "PI credentials and institution", "required": True},
            ...
            {"field": "investigator_signature", "label": "Investigator/designee signature", "required": True},
        ],
        "warnings": [
            ...
            "No dosing, injection, formulation, or manufacturing guidance is included.",
            "NeoVax-Agent cannot produce approved consent forms — this is a coordination scaffold only.",
        ],
        "case_id": case_id,
        "candidate_count": candidate_count,
        "irb_required": True,
