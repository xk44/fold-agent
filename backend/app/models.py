...
class Case(Base):
    ...
    supervising_professional: Mapped[str | None] = mapped_column(String(500), nullable=True)
    ...
    candidates: Mapped[list["CandidateAntigen"]] = relationship(
        back_populates="case", cascade="all, delete-orphan"
    )
    structure_jobs: Mapped[list["StructureJob"]] = relationship(
        back_populates="case", cascade="all, delete-orphan"
    )
...
class Sample(Base):
    ...
    source_lab: Mapped[str | None] = mapped_column(String(500), nullable=True)
    custody_metadata: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    created_at: Mapped[datetime] = mapped_column(DateTime, default=lambda: datetime.now(UTC))
...
class Variant(Base):
    ...
    transcript: Mapped[str | None] = mapped_column(String(100), nullable=True)
    protein_change: Mapped[str | None] = mapped_column(String(200), nullable=True)
    caller_source: Mapped[str | None] = mapped_column(String(200), nullable=True)
    quality_metrics: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    annotation_source: Mapped[str | None] = mapped_column(String(200), nullable=True)
    expert_review_notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    last_parsed_execution_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
...
class CandidateAntigen(Base):
    ...
    peptide_metadata: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    mhc_context: Mapped[str | None] = mapped_column(String(500), nullable=True)
    prediction_scores: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    expression_evidence: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    structure_evidence: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    uncertainty_flags: Mapped[dict | None] = mapped_column(JSON, nullable=True)
    expert_review_notes: Mapped[str | None] = mapped_column(Text, nullable=True)
    last_parsed_execution_id: Mapped[str | None] = mapped_column(String(36), nullable=True)
    review_status: Mapped[ReviewStatusEnum] = mapped_column(
        Enum(ReviewStatusEnum), default=ReviewStatusEnum.unreviewed
...
class StructureJob(Base):
    ...
    confidence_metrics: Mapped[dict | None] = mapped_column(JSON, nullable=True)
...
class PipelineRun(Base):
    ...
    status: Mapped[PipelineStatusEnum] = mapped_column(
        Enum(PipelineStatusEnum), default=PipelineStatusEnum.pending
    )
...
class Artifact(Base):
    ...
    execution_run_id: Mapped[str | None] = mapped_column(
        ForeignKey("execution_runs.id"), nullable=True
    )
...
class AuditLog(Base):
    ...
    outputs_hash: Mapped[str | None] = mapped_column(String(128), nullable=True)
