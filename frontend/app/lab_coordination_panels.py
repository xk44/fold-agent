...
def _render_contacts(api_base: str, case_id: str, http_get: Any, http_post: Any, st: Any) -> None:
    ...
    with st.expander("Add Contact"):
        ...
        if st.button("Add Contact") and name:
            http_post(
                f"{api_base}/cases/{case_id}/lab/contacts",
                json={
                    "name": name,
                    "role": role,
                    "organization": org,
...
def _render_costs(api_base: str, case_id: str, http_get: Any, http_post: Any, st: Any) -> None:
    ...
    with st.expander("Add Cost Entry"):
        ...
        status = st.selectbox(
            "Status", ["estimated", "quoted", "invoiced", "paid"], key="cost_status"
        )

        if st.button("Add Cost") and desc:
            http_post(
                f"{api_base}/cases/{case_id}/lab/costs",
                json={
                    "category": category,
                    "description": desc,
...
def _render_timeline(api_base: str, case_id: str, http_get: Any, http_post: Any, st: Any) -> None:
    ...
    with st.expander("Add Timeline Entry"):
        ...
        status = st.selectbox(
            "Status", ["pending", "in_progress", "completed", "blocked"], key="tl_status"
        )

        if st.button("Add Entry") and title:
            http_post(
                f"{api_base}/cases/{case_id}/lab/timeline",
                json={
                    "title": title,
                    "owner": owner,
                    "status": status,
...
def _render_document_requests(
    ...
    with st.expander("New Request"):
        ...
        if st.button("Submit Request") and doc_type:
            http_post(
                f"{api_base}/cases/{case_id}/lab/document-requests",
                json={
    ...
    if requests:
        for r in requests:
            status_color = {"pending": "🟡", "received": "🟢", "overdue": "🔴"}.get(
                r["status"], "⚪"
            )
